        ...
        >
            <li v-for="(item, index) in list.data" :key="index">
                <div class="l-info">
                    ...
                    <p class="flex aic width-160">
                        <span>链接：</span>
                        <el-input v-model="item.link" placeholder="请输入连接" size="mini" clearable></el-input>
...
export default {
    ...
    methods: {
        ...
        async upload(params) {
            ...
            if (len >= this.len && this.imageIndex) {
            ...
            if (index !== null) {
                this.data.data[index].url=url;
                this.data.data[index].name = file.name;
            } else {
